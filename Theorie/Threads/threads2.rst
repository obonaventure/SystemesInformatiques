...
Même si les threads sont très utiles dans des ordinateurs multiprocesseurs, ils ont été inventés et utilisés d'abord sur des processeurs capables d'exécuter un seul thread d'exécution à la fois. Sur un tel processeur, les threads d'exécution sont entrelacés plutôt qu'exécutés réellement simultanément. Cet entrelacement est réalisé par le système d'exploitation.
...
Sans entrer dans des détails qui relèvent du fonctionnement interne des systèmes d'exploitation, on peut schématiquement représenter un :term:`mutex` comme étant une structure de données qui contient deux informations :
