...
Cette solution utilise trois mutex, deux sémaphores et deux variables partagées : ``readcount`` et ``writecount``. Ces deux variables servent respectivement à compter le nombre de `readers` qui accèdent à la base de données et le nombre de `writers` qui veulent y accéder. Le sémaphore ``wsem`` est utilisé pour assurer un accès exclusif à la base de données lorsqu'un `writer` est occupé à écrire (il bloque à la fois les autres `writers` et `readers`). Le sémaphore ``rsem`` permet aux `writers` de bloquer l'entrée de nouveaux `readers`. Le mutex ``z`` a un rôle particulier qui sera plus clair lorsque le code des `readers` aura été présenté. Les deux sémaphores sont initialisés à la valeur ``1``.
